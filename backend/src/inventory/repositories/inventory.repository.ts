...
type InventoryCopyStatus = "DISPONIBLE" | "PRESTADO" | "NO_DISPONIBLE";
type InventoryCopyStateManaged = Extract<InventoryCopyStatus, "DISPONIBLE" | "NO_DISPONIBLE">;
...
export class InventoryRepository {
  ...
  async createCategory(data: { nombre: string; tiempoMaximoPrestamoDias: number }) {
    const query = this.prisma.sql.public.categorias_recurso
      .insert([{ nombre: data.nombre, tiempoMaximoPrestamoDias: data.tiempoMaximoPrestamoDias }])
      .returning("id", "nombre", "tiempoMaximoPrestamoDias", "createdAt", "updatedAt")
      .build();
  ...
  async findCategoryById(categoryId: string) {
    const query = this.prisma.sql.public.categorias_recurso
      .select("id", "nombre", "tiempoMaximoPrestamoDias", "createdAt", "updatedAt")
  ...
  async updateCategory(categoryId: string, data: Partial<{ nombre: string; tiempoMaximoPrestamoDias: number }>) {
    const query = this.prisma.sql.public.categorias_recurso
      .update({
        ...(data.nombre !== undefined ? { nombre: data.nombre } : {}),
        ...(data.tiempoMaximoPrestamoDias !== undefined ? { tiempoMaximoPrestamoDias: data.tiempoMaximoPrestamoDias } : {}),
      })
      .where((row, fns) => fns.eq(row.id, categoryId))
      .returning("id", "nombre", "tiempoMaximoPrestamoDias", "createdAt", "updatedAt")
      .build();
  ...
  async createResource(data: { categoriaId: string; nombre: string; descripcion?: string | null }) {
    const query = this.prisma.sql.public.recursos
      .insert([
        {
      ...
      .returning("id", "categoriaId", "nombre", "descripcion", "createdAt", "updatedAt")
      .build();
    const rows = await this.runtime().query(query);
    return rows[0] ?? null;
  }

  async findAllResources(filters?: { categoryId?: string; available?: boolean; search?: string }) {
    ...
    if (filters?.categoryId) {
      base = base.where((row, fns) => fns.eq(row.categoriaId, filters.categoryId!));
    }

    if (filters?.search) {
      base = base.where((row, fns) => fns.ilike(row.nombre, `%${filters.search!}%`));
    }
  ...
  async findResourceById(resourceId: string) {
    const query = this.prisma.sql.public.recursos
      .select("id", "categoriaId", "nombre", "descripcion", "createdAt", "updatedAt")
  ...
  async updateResource(resourceId: string, data: Partial<{ categoriaId: string; nombre: string; descripcion?: string | null }>) {
    const query = this.prisma.sql.public.recursos
      .update({
  ...
  async createCopy(data: { recursoId: string; codigoInventario: string; estado?: InventoryCopyStatus }) {
    const estado: InventoryCopyStatus = (data.estado ?? "DISPONIBLE") as InventoryCopyStatus;
    const query = this.prisma.sql.public.ejemplares
      .insert([
        {
      ...
      .returning("id", "recursoId", "codigoInventario", "estado", "createdAt", "updatedAt")
      .build();
    const rows = await this.runtime().query(query);
    return rows[0] ?? null;
  }
  ...
  async findCopiesByResource(resourceId: string) {
    const query = this.prisma.sql.public.ejemplares
      .select("id", "recursoId", "codigoInventario", "estado", "createdAt", "updatedAt")
      .where((row, fns) => fns.eq(row.recursoId, resourceId))
      .build();
    return await this.runtime().query(query);
  }
  ...
  async updateCopyState(copyId: string, estado: InventoryCopyStateManaged) {
    const nextState: InventoryCopyStateManaged = estado as InventoryCopyStateManaged;
    const query = this.prisma.sql.public.ejemplares
