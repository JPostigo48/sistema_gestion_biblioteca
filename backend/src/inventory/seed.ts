...
async function queryRows<T = any>(plan: unknown): Promise<T[]> {
  return (await prisma.client.runtime().query(plan as any)) as T[];
}
...
async function upsertCategory(nombre: string, tiempoMaximoPrestamoDias: number) {
...
async function upsertResource(categoriaId: string, nombre: string, descripcion: string) {
...
async function upsertCopy(recursoId: string, codigoInventario: string, estado: "DISPONIBLE" | "NO_DISPONIBLE") {
...
async function createObservation(ejemplarId: string, descripcion: string) {
...
async function seedInventory() {
  ...
  try {
    ...
    for (const item of categorias) {
      const categoria = await upsertCategory(item.nombre, item.tiempoMaximoPrestamoDias);
    ...
    const recursos = [
      {
        categoria: "Libros",
        nombre: "Clean Code",
        descripcion: "Libro de referencia sobre calidad y legibilidad del código.",
      },
      {
        categoria: "Libros",
    ...
    for (const item of recursos) {
      ...
      const resource = await upsertResource(categoria.id, item.nombre, item.descripcion);
    ...
    const copySeed = [
      ...
      { key: "Libros:Patrones de Diseño", codigo: "PD-001", estado: "DISPONIBLE" },
      { key: "Revistas:IEEE Software", codigo: "SW-001", estado: "DISPONIBLE" },
      { key: "Tesis:Metodología de Investigación", codigo: "MI-001", estado: "NO_DISPONIBLE" },
    ] as const;
    ...
    for (const item of copySeed) {
      ...
      if (copy) {
        ...
        await createObservation(copy.id, descripcion);
      }
    }
