...
describe("InventoryController", () => {
  it("should delegate category creation to the service", async () => {
    const service = {
      createCategory: vi.fn().mockResolvedValue({ id: "cat-1", nombre: "Libros" }),
    };

    const controller = new InventoryController(service as any);
    const result = await controller.createCategory({ nombre: "Libros", tiempoMaximoPrestamoDias: 14 });

    expect(service.createCategory).toHaveBeenCalledWith({ nombre: "Libros", tiempoMaximoPrestamoDias: 14 });
